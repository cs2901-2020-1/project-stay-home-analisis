...
public interface LikesPorArticleRepository extends JpaRepository<LikesPorArticle, Dbi> {
    ...
    @Transactional
    @Query(value = "SELECT * FROM app_likesbyarticle WHERE app_likesbyarticle.articleid = :id AND app_likesbyarticle.like = TRUE", nativeQuery = true)
    List<LikesPorArticle> getLikesporArticle(@Param("id") BigInteger id);

    @Query(value = "SELECT * FROM app_likesbyarticle WHERE app_likesbyarticle.articleid = :id AND app_likesbyarticle.like = FALSE", nativeQuery = true)
